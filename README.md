...
등록이 모두 끝나면 번호가 붙은 파일을 ZIP으로 준비합니다. 한 ZIP이 1,850MB를 넘을 경우 `part01`, `part02`처럼 자동 분할하며, 라이브러리의 **Send ZIP** 버튼을 눌렀을 때 텔레그램 채팅으로 전송합니다. 기존 시리즈는 Send ZIP을 처음 누를 때 백그라운드에서 ZIP을 생성합니다.
